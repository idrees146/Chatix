...
const Page = () => {
  ...
  return (
    <div className="flex flex-col h-screen w-full">
      <div className="profile p-2 flex items-center justify-between gap-4 h-16 bg-green-900 sticky top-0 z-50 shadow-md">
        {session && (
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-2">
        {messages.map((msg) => (
          <div
            key={msg._id}
            className={`${
              msg.email === sender ? 'ml-auto bg-blue-500' : 'mr-auto  bg-green-800'
            } text-white p-3 max-w-xs md:max-w-md rounded-lg my-6`}
          >
            {msg.message}
          </div>
        ))}
